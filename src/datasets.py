...
class GeonodeDatasets(GeoNodeObject):
  ...
  def upload(self,
    ...
    Args:
        filepath_path (Path): Path to the file to upload. If shape make sure to set the shp file and add place other files with same name next to the given
    ...
    if dataset_path.suffix == '.shp':
      dbf_file = Path(os.path.join(dataset_path.parent, dataset_path.stem + ".dbf"))
      shx_file = Path(os.path.join(dataset_path.parent, dataset_path.stem + ".shx"))
      prj_file = Path(os.path.join(dataset_path.parent, dataset_path.stem + ".prj"))

      if not any(x.exists for x in [dataset_path, dbf_file, shx_file, prj_file]):
        raise FileNotFoundError

      content_length = sum([os.path.getsize(f) for f in [dataset_path, dbf_file, shx_file, prj_file]])

      files = [
          ('base_file', (dataset_path.name, open(dataset_path, 'rb'), 'application/octet-stream')),
          ('dbf_file', (dbf_file.name, open(dbf_file, 'rb'), 'application/octet-stream')),
          ('shx_file', (shx_file.name, open(shx_file, 'rb'), 'application/octet-stream')),
          ('prj_file', (prj_file.name, open(prj_file, 'rb'), 'application/octet-stream')),
        ]
